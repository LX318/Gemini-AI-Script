...
local UI = HoloLib:CreateWindow({ Title = "🍎 My Script Hub" })

UI:CreateLabel("🛠️ 基础测试区")
UI:CreateButton("点击测试按钮", function() print("按钮被点击！") end)
UI:CreateToggle("功能开关", false, function(state) print("当前状态:", state) end)
UI:CreateSlider("测试数值滑块", 0, 100, 50, function(value) print("滑块数值:", value) end)
UI:CreateInput("在这里输入文字然后按回车...", function(text) print("你输入了:", text) end)

local SubPanel = UI:CreateSubTab("➡️ 打开侧边面板")
SubPanel:CreateLabel("这是你的独立工作区")
SubPanel:CreateToggle("自动功能示例", false, function(state) end)

local GalleryTab = UI:CreateSubTab("🖼️ 看图专用")
GalleryTab:CreateLabel("支持: 1.数字ID 2.Github RAW直链")
local UpdateMyImage = GalleryTab:CreateImageDisplay()
GalleryTab:CreateInput("粘贴 URL 或 资产ID... (Enter确认)", function(text)
    if text and text ~= "" then UpdateMyImage(text) end
end)
